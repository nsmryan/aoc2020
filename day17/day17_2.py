...
offsets = [(x, y, z, w) for x in range(-1, 2) for y in range(-1, 2) for z in range(-1, 2) for w in range(-1, 2)]
...
def step_board(board):
    ...
    for w in range(mini[2] - 1, maxi[2] + 2):
...
def solve(board, iters=6):
    for i in range(0, iters):
        print("iter = " + str(i) + " active " + str(count_active(board)))
